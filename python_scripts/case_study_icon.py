...
def plot_wp(t, wp, iwp, lwp, xlim=[None,None], ylim=[None,None], figsize=(10,4), save=False):
    ...
    ax[0].set_xlim(xlim)
    ax[0].set_ylim(ylim)
    fig.suptitle("time step = "+str(t)+" "+(util.tstring(t*0.25)))
    if save:
        if DC:
            plt.savefig("../plots/icon/case_study_"+region+"_i{0:03d}.png".format(t), bbox_inches="tight")
        else:
            plt.savefig("../plots/icon/case_study_"+region+"_ci_i{0:03d}.png".format(t), bbox_inches="tight")
    plt.show()

# %%
# load frozen and liquid water paths
iwp = load01deg.get_iwp(model, region, ice_only=False)
lwp = load01deg.get_lwp(model, region, rain=False)
