...
def plot_wp(t, wp, iwp, lwp, xlim=[None,None], ylim=[None,None], figsize=(10,4), save=False):
    ...
    # pc = ax[0].pcolormesh(twp.lon, twp.lat, np.log10(wp[t]*1000), cmap="gist_earth_r",
    #                         vmax=5, vmin=-1)
    # cbar = plt.colorbar(pc, ax=ax[0])
    # cbar.set_label("log (g/m2)")
    # ax[0].set_title("water path (g/m2)")

    pc = ax[0].pcolormesh(twp.lon, twp.lat, np.log10(iwp[t]*1000), cmap="gist_earth_r",
                            vmax=5, vmin=-1)
    cbar = plt.colorbar(pc, ax=ax[0])
    cbar.set_label("log (g/m2)")
    ax[0].set_title("frozen water path (g/m2)")

    pc = ax[1].pcolormesh(twp.lon, twp.lat, np.log10(lwp[t]*1000), cmap="gist_earth_r",
                            vmax=5, vmin=-1)
    ...
    if save:
        if DC:
            plt.savefig("../plots/nicam/case_study_"+region+"_n{0:03d}.png".format(t), bbox_inches="tight")
        else:
            plt.savefig("../plots/nicam/case_study_"+region+"_ci_n{0:03d}.png".format(t), bbox_inches="tight")
    plt.show()
...
iwp = load.get_iwp(model, region, ice_only=False)[:,0]
lwp = load.get_lwp(model, region, rain=False)[:,0]
twp = iwp + lwp
