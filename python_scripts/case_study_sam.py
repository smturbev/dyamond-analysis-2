...
def plot_wp(t, wp, iwp, lwp, xlim=[None,None], ylim=[None,None], figsize=(10,4), save=False):
    ...
    if save:
        if DC:
            plt.savefig("../plots/sam/case_study_"+region+"_s{0:03d}.png".format(t), bbox_inches="tight")
        else:
            plt.savefig("../plots/sam/case_study_"+region+"_ci_s{0:03d}.png".format(t), bbox_inches="tight")
    plt.show()
...
if DC:
    t = times_dc[0]
    print(t)
    plot_wp(t, twp_dc, iwp, lwp, save=False)
    print("done")
else:
    t = 962
    print(t)
